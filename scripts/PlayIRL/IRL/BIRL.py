...
class BIRLF(threading.Thread):
    ...
    def soft_transition(self, itr, new_rl_model, new_omega, new_qvalues_D, condition=0):
        new_log_posterior = self.calc_posterior(itr, new_rl_model, new_omega, new_qvalues_D, None)
        if sample_prob(min(1, np.exp(new_log_posterior-self.log_posterior))):
            if condition == 0:
                print('transition to new model condition 0')
                return new_rl_model, new_omega, new_log_posterior
            else:
                print('transition to new model condition 1')
                return self.rl_model, new_omega, new_log_posterior
        print('remain old model')
        return self.rl_model, self.omega, self.log_posterior


    def hard_transition(self, itr, new_rl_model, new_omega, new_qvalues_D, new_halfplane_normals):
        ...
        new_log_posterior = self.calc_posterior(itr, new_rl_model, new_omega, new_qvalues_D, new_halfplane_normals)
    ...
    def run(self):
        ...
        # self.log_posterior = self.calc_posterior(0, self.rl_model, self.omega, qvalues_D, halfplane_normals)
        # for irl_iter in range(self.irl_config.episodes_num):
        #     new_rl_model = self.copy_to_new_model(self.rl_model)
        ...
        #         self.rl_model, self.omega, self.log_posterior = self.hard_transition(irl_iter, new_rl_model, new_omega, new_qvalues_D, new_halfplane_normals)
        #     else:
        #         self.rl_model, self.omega, self.log_posterior = self.hard_transition(irl_iter, new_rl_model, new_omega, new_qvalues_D, new_halfplane_normals)
    ...
    def generate_halfplane_normals_pi(self, rl_model, demos):
        ...
        sampled_delta_mus /= np.maximum(np.linalg.norm(sampled_delta_mus, axis=1, keepdims=True), 1e-6)
    ...
    def generate_halfplane_normals_demos(self, rl_model, demos):
        ...
        demo_delta_mus /= np.maximum(np.linalg.norm(demo_delta_mus, axis=1, keepdims=True), 1e-6)
    ...
    def calc_posterior(self, itr, rl_model, omega, qvalues, halfplane_normals_demo):
        log_posterior = 0.0
        ...
        log_likelihood = self.irl_config.gamma*np.sum(qvalues)+ \
            self.irl_config.gamma_infogap*(infogap_D-infogap_F)+ \
            gap_scale*self.irl_config.gamma_infogap*abs(infogap_D-infogap_F)

        # log_likelihood = self.irl_config.gamma*np.sum(qvalues)+ \
        #     self.irl_config.gamma_infogap*infogap_D+ \
        #     self.irl_config.gamma_infogap*(infogap_D-infogap_F)*(infogap_D > infogap_F)
